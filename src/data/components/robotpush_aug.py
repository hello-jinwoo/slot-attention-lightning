...
class RobotPushAug(Dataset):
    def __init__(
        self,
        data_dir: str = "data/robotpush_rgb",
        img_size: int = 128,
        crop_size: int = 196,
        ...
        self.image_dir = os.path.join(data_dir, self.stage, "256")
    ...
    def __getitem__(self, index):
        ...
        if not self.train:
            ...
            sample["masks_ori"] = torch.zeros(self.max_num_masks, self.img_size, self.img_size, 1)
            sample["masks_aug"] = torch.zeros(self.max_num_masks, self.img_size, self.img_size, 1)
