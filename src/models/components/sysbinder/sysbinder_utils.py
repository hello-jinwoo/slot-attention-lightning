...
from src.models.components.sysbinder.transformer import TransformerEncoder, TransformerDecoder
...
class ImageEncoder(nn.Module):
    def __init__(self,
        ...
        self.cnn = nn.Sequential(
            Conv2dBlock(image_channels, cnn_hidden_size, 5, 1 if image_size == 64 else 2, 2),
...
class ImageDecoder(nn.Module):
    def __init__(self,
        ...
        self.slot_proj = BlockLinear(slot_size, d_model * num_blocks, num_blocks)

        self.block_pos = nn.Parameter(torch.zeros(1, 1, d_model * num_blocks), requires_grad=True)
        self.block_pos_proj = nn.Sequential(
            BlockLinear(d_model * num_blocks, d_model * num_blocks, num_blocks),
            nn.ReLU(),
            BlockLinear(d_model * num_blocks, d_model * num_blocks, num_blocks)
        )

        self.block_coupler = TransformerEncoder(num_blocks=1, d_model=d_model, num_heads=4)
        ...
        self.decoder_pos = LearnedPositionalEmbedding1D(1 + (image_size // 4) ** 2, d_model)

        self.tf = TransformerDecoder(
            num_decoder_layers, (image_size // 4) ** 2, d_model, num_decoder_heads, dropout)
